...
class NeighborInfoAppProcessor(Processor):
    def process(self, payload: bytes, client_details: ClientDetails, **kwargs):
        ...
        try:
            ...
            return
        self.update_node_neighbors(neighbor_info, client_details, kwargs.get('mesh_packet'))

    def update_node_neighbors(self, neighbor_info: NeighborInfo, client_details: ClientDetails, mesh_packet=None):
        def operation(cur, conn):
            ...
            for neighbor in neighbor_info.neighbors:
                ...
                cur.execute("""
                    INSERT INTO node_neighbors (time, node_id, neighbor_id, snr, packet_id, rx_time)
